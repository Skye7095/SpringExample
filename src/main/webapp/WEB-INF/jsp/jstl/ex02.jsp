...
<head>
<meta charset="UTF-8">
<title>JSTL Core 라이브러리2 - 조건문 / 반복문</title>
</head>
<body>
	...
	<%-- varStatus는 count, index, first, last를 갖고 올 수 있음. --%>
	<%-- count는 1부터 시작. index는 0부터 시작. first는 반복값 1번인지, last는 반복값 마지막인지 확인. --%>
	<c:forEach var="fruit" items="${fruitList }" varStatus="status">
		<h4>${fruit } ::: ${status.count } ${status.index } ${status.first } ${status.last }</h4>
	</c:forEach>
	
	<%-- map형식인 list를 출력하는 방법. > table로 표현함 --%>
	<table border="1">
		<thead>
			<tr>
				<th>이름</th>
